...
def analizar_codigo(codigo_prueba):
    PALABRAS = ['if','else','print','prnt','pront']
    TOKENS_VALIDOS = {'if':'TK_IF','else':'TK_ELSE','print':'TK_PRINT','prnt':'TK_PRINT','pront':'TK_PRINT'}
    sugerencias = []
    tokens = []
        ...
        if lexema in TOKENS_VALIDOS or re.match(r'^[a-zA-Z_]\w*$', lexema):
            if lexema in TOKENS_VALIDOS:
                ...
                if lexema == 'pront':
                    # corregir a 'print'
                    sugerencia = 'print'
                    sugerencias.append(f"Sugerencia: '{lexema}' → '{sugerencia}'")
    ...
    while pos < len(tokens):
        ...
        if valor in ['print']:
            ...
            if pos < len(tokens) and tokens[pos][1] == '~':
                consumir()
            cuerpo_if = None
            if pos < len(tokens) and tokens[pos][1] in ['print']:
                consumir()
                args_if = []
                while pos < len(tokens) and tokens[pos][1] not in ['else',';']:
...
if __name__ == '__main__':
    codigo = 'pront $x=5$; if $x>3~prnt(x) else prnt("no")'
